...
public:
    ...
    bool ShowCase(void)
    {
        ...
        if (primary_owner && primary_owner != window) {
            ...
            if (!ConvertSelection(window, GetAtom("PRIMARY"), GetAtom("TARGETS"), GetAtom("PRIMARY_TARGETS"))) {
                return false;
            }
        }
        ...
        if (secondary_owner && secondary_owner != window) {
            // Case 2-1. request available targets aka 'mime_types' from the selection owner
            if (!ConvertSelection(window, GetAtom("SECONDARY"), GetAtom("TARGETS"), GetAtom("SECONDARY_TARGETS"))) {
                return false;
            }
        }
        ...
        if (clipboard_owner && clipboard_owner != window) {
            // Case 2-1. request available targets aka 'mime_types' from the selection owner
            if (!ConvertSelection(window, GetAtom("CLIPBOARD"), GetAtom("TARGETS"), GetAtom("CLIPBOARD_TARGETS"))) {
                return false;
            }
        }
    ...
    bool ProcSelectionNotify(xcb_selection_notify_event_t *event)
    {
        ...
        if (event->target == GetAtom("TARGETS")) {  // Case 2-2
            ...
            for (uint32_t i = 0; i < reply->length; i++) {
                ...
                snprintf(property_name, sizeof(property_name) - 1, "%s_%s", GetAtomName(event->selection), GetAtomName(mime_types[i]));
                if (!ConvertSelection(window, event->selection, mime_types[i], GetAtom(property_name))) {
                    return false;
                }
            }
            free(reply);
